...
class FeedForwardDDPG(nn.Module):
    def __init__(self, num_inputs, num_outputs, hidden_nodes, device, std=0.1):
        super(FeedForwardDDPG, self).__init__()
        self.device = device
        ...
        for i in range(len(hidden_nodes) - 1):
            if i == 0:
                self.critic.append(nn.Linear(hidden_nodes[i] + num_outputs, hidden_nodes[i + 1]))
            else:
                self.critic.append(nn.Linear(hidden_nodes[i], hidden_nodes[i + 1]))
            self.critic.append(act_func())
    ...
    def get_value(self, state, actions):
        ...
        values = state
        for layer in self.critic:
            if layer == self.critic[2]:
                values = torch.cat([values, actions],1)
            values = layer(values)
    ...
    def get_logp_value_ent(self, state, actions_prev, std):
        ...
        actions_cur_p = self.get_action(state) # Actions on current policy
        values = self.get_value(state, actions_cur_p)
